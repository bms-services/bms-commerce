...
import { useChannelBranding } from "@/providers/ChannelBrandingProvider";
...
export default function LogoIcon({ className, showName = true }: LogoIconProps) {
  const { name, logoUrl } = useChannelBranding();

  return (
    <div className={className}>
      <div className="flex items-center gap-x-2">
        {logoUrl ? (
          <div className="relative !min-h-10 !min-w-10 h-10 w-10 shrink-0 overflow-hidden rounded-lg border border-border-muted bg-white dark:border-dark-grey dark:bg-black">
            <Image
              src={logoUrl}
              alt={name}
              fill
